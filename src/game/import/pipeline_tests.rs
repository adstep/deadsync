...
fn imports_stats_xml_against_library_end_to_end() {
    ...
    for s in &songs {
        for steps in &s.steps {
            for hs in &steps.high_scores {
                total += 1;
                match resolver.resolve(&s.dir, &steps.steps_type, &steps.difficulty, &steps.description)
                {
    ...
    assert_eq!(entry.ex_score_percent, 0.0, "EX not recoverable from Stats.xml");
