...
mod tests {
    ...
    fn parses_spacing_and_visual_delay() {
        let base = PlayerOptionsData::default();
        let out = translate_player_options(
            &sl(&[("Spacing", "-25%"), ("VisualDelay", "12ms")]),
            &base,
        );
    ...
    fn translates_stock_graphics_and_font() {
        ...
        assert_eq!(out.judgment_graphic.as_str(), "judgements/Wendy 2x7 (doubleres).png");
        assert_eq!(
    ...
    fn translates_enum_settings() {
        ...
        assert_eq!(out.background_filter, BackgroundFilter::from_str("50").unwrap());
    ...
    fn translates_step_stats_extra_and_target_score() {
        ...
        let specified = translate_player_options(
            &sl(&[("TargetScore", "SpecifiedValue"), ("TargetScoreNumber", "95")]),
            &base,
        );
    ...
    fn translates_data_visualizations_legacy_values() {
        ...
        let stats = translate_player_options(
            &sl(&[("DataVisualizations", "Step Statistics")]),
            &base,
        );
        assert_eq!(stats.step_statistics, StepStatisticsMask::all_widgets());

        let none = translate_player_options(
            &sl(&[("DataVisualizations", "Target Score Graph")]),
            &base,
        );
    ...
    fn translates_error_bar_flags() {
        ...
        assert_eq!(out.error_bar, error_bar_style_from_mask(out.error_bar_active_mask));
    ...
    fn translates_column_flash_flags_splitting_fantastic() {
        ...
        assert!(out.column_flash_mask.contains(
            ColumnFlashMask::BLUE_FANTASTIC | ColumnFlashMask::WHITE_FANTASTIC
        ));
