...
pub fn clear_navigation_holds(state: &mut State) {
...
pub fn on_lr_press(state: &mut State, delta: isize) {
    let now = Instant::now();
...
pub fn on_lr_release(state: &mut State, delta: isize) {
...
pub fn apply_submenu_choice_delta(
    state: &mut State,
...
pub fn cancel_current_view(state: &mut State) -> ScreenAction {
    match state.view {
...
pub fn undo_three_key_selection(state: &mut State, asset_manager: &AssetManager) {
...
pub fn activate_current_selection(state: &mut State, asset_manager: &AssetManager) -> ScreenAction {
    match state.view {
        OptionsView::Main => {
            let total = ITEMS.len();
            if total == 0 {
