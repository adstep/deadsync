...
impl From<&SpeedMod> for ScrollSpeedSetting {
    fn from(sm: &SpeedMod) -> Self {
        match sm.mod_type {
            SpeedModType::X => Self::XMod(sm.value),
            SpeedModType::C => Self::CMod(sm.value),
            SpeedModType::M => Self::MMod(sm.value),
        }
    }
}

impl From<ScrollSpeedSetting> for SpeedMod {
    fn from(s: ScrollSpeedSetting) -> Self {
        match s {
            ScrollSpeedSetting::XMod(v) => Self { mod_type: SpeedModType::X, value: v },
            ScrollSpeedSetting::CMod(v) => Self { mod_type: SpeedModType::C, value: v },
            ScrollSpeedSetting::MMod(v) => Self { mod_type: SpeedModType::M, value: v },
        }
    }
}
...
pub fn sync_profile_scroll_speed(profile: &mut crate::game::profile::Profile, speed_mod: &SpeedMod) {
    profile.scroll_speed = speed_mod.into();
}
