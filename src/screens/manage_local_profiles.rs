...
/// added.
fn merge_import_candidates(picker: &mut ImportPickerState, found: Vec<ItgProfileCandidate>) -> usize {
...
fn poll_folder_pick(state: &mut State) {
    ...
    let picker = state.import_picker.get_or_insert_with(|| ImportPickerState {
        candidates: Vec::new(),
        imported_as: Vec::new(),
        selected: 0,
        info: None,
...
fn select_profile_row(state: &mut State, profile_id: &str) {
    if let Some(pos) = state.rows.iter().position(|r| {
        matches!(&r.kind, RowKind::Profile { id, .. } if id == profile_id)
    }) {
        state.selected = pos;
        state.prev_selected = pos;
    }
...
fn import_summary_message(summary: &ImportSummary) -> ImportMessageState {
    ...
    lines.push(MessageLine::plain(
        tr_fmt("Profiles", "ImportSummaryName", &[("name", &summary.display_name)]).to_string(),
    ));

    // Scores: imported/total, amber when some were skipped.
    let scores_skipped = summary.charts_song_not_found
        + summary.charts_chart_not_found
        + summary.scores_unmapped;
...
fn bool_row(label_key: &str, on: bool, done_key: &str, none_key: &str) -> MessageLine {
    if on {
        section_row(label_key, tr("Profiles", done_key).to_string(), SectionStatus::Imported)
    } else {
        section_row(label_key, tr("Profiles", none_key).to_string(), SectionStatus::Skipped)
    }
}
...
fn push_import_picker_overlay(ui: &mut Vec<Actor>, state: &State, asset_manager: &AssetManager) {
    ...
    let widest_label = measure_label_width(asset_manager, &max_name_ref, ROW_ZOOM).max(
        measure_label_width(asset_manager, &tr("Profiles", "ImportBrowseButton"), ROW_ZOOM),
    );
    ...
    push_popup_backdrop(ui, w, h);
    push_popup_box(ui, cx, cy, box_w, box_h);
    push_popup_title(ui, tr("Profiles", "ImportPickTitle").to_string(), cx, top, box_w - 40.0);
