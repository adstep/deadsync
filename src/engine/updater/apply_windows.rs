        ...
        let target_existed = match fs::symlink_metadata(&target) {
            Ok(m) => {
                if !m.is_file() {
                    ...
                    // reparse point with a regular file. The execute
                    // phase's `rename` would otherwise move the entire
                    // subtree aside as the "backup", and the
                    // `Applied` cleanup would then fail trying to
                    // `remove_file` it, wedging the journal forever.
                    return Err(UpdaterError::Io(format!(
