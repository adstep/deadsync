...
| C2  | Gate `DEADSYNC_UPDATER_FAKE_DOWNLOAD` to dev/test builds      | 🔴       | ✅ Done        | Resolved by removal: the `DEADSYNC_UPDATER_FAKE_DOWNLOAD[_SECS]` shortcut and its `run_fake_download` path were removed from `action.rs` entirely; tests use `MockHttpClient` instead. |
...
| M6  | Gate `DEADSYNC_UPDATER_RELEASE_URL` to dev/test builds        | 🟠       | ❎ Won't fix   | Override intentionally retained unconditionally for ongoing manual end-to-end testing against fork releases. Fake-download path was removed (C2), so the URL override is the only remaining test hook and only redirects which release manifest is fetched — sidecar + API digest still gate `Ready`. |
...
| C9  | Env-var release URL & fake-download active in release builds  | 🔴       | ⚠️ Partial     | Fake-download removed entirely (see C2). `DEADSYNC_UPDATER_RELEASE_URL` is intentionally retained per maintainer decision (see M6) — it only redirects which release manifest is fetched and is the sole remaining hook for manual e2e testing against fork releases; sidecar + API digest still gate `Ready`. The `DEADSYNC_NO_UPDATE_CHECK` opt-out was also removed; `--no-update-check` covers that use case. |
...
| M27 | Apply success + relaunch failure looks like apply failure     | 🟠       | ✅ Done | If `apply_archive_and_relaunch` succeeds at apply but fails to spawn (`cli.rs:143-147`, `action.rs:665-674`), the current old process keeps running against a mutated install tree and an `Applied` journal. Distinguish the two outcomes: on apply-ok-relaunch-fail, publish a phase that says "Update installed; please restart manually" (and ideally `process::exit` rather than continue). |
...
| M32 | `run_check_once` reads cache twice without holding the lock   | 🟠       | ✅ Done        | `run_check_once` now captures `let prev_cache = cache();` once at the top and uses it both for the `If-None-Match` ETag and as the baseline passed into `apply_fresh_to_cache`. The previous double-read could let a racing `write_cache` swap the baseline out from under the second read; safe today (only the startup worker calls it) but no longer fragile against future concurrent callers. |
| M33 | `Applying` overlay phase falls into `_` catch-all             | 🟡       | ✅ Done        | `update_overlay::handle_input` now lists every `ActionPhase` variant explicitly (including `Applying { .. } => Consumed` and `Idle => Passthrough`) instead of relying on a final `_ => Consumed` arm. Adding a future variant is now a compile error here rather than silently inheriting swallow-all behaviour. |
| M34 | `ProgressThrottle` rate-limits indeterminate streams to 10 Hz | 🟡       | ✅ Done        | Investigated and confirmed intentional. For `total = None` the overlay shows a byte counter (no percent bar), and the 10 Hz floor driven by the 100 ms timer is plenty for a numeric readout. Tightened the `should_publish` doc comment to call out indeterminate-stream semantics explicitly, and added `progress_throttle_advances_at_10hz_for_indeterminate_stream` to lock in the cadence so a future change can't silently regress it. |
| M35 | New updater dependencies pull default features                | 🟡       | ✅ Done        | Audited via `cargo tree --edges features`. `ureq[json]` is genuinely used (`send_json` in `network.rs` / `groovestats.rs` / `arrowcloud.rs`), keep. `flate2`'s `rust_backend` default is the pure-Rust `miniz_oxide` backend we want, keep. Switched `zip` from `["deflate"]` to `["deflate-flate2-zlib-rs"]` to drop the slow `zopfli` encoder we never use (we only decompress). Disabled `tar` defaults to drop the Linux-only `xattr` dep we don't read or write. 160 updater tests still pass, including the zip-extraction smoke test. |
| N7  | Swedish locale missing `BodyManualDownload`                   | 🟠       | ⏳ Not started | `assets/languages/sv.ini` has no `[Updater] BodyManualDownload` key; `en.ini` and `pseudo.ini` do. When the active locale is Swedish and the overlay enters `AvailableNoInstall` (e.g. on macOS or `UpdaterInstallEnabled = 0`), the lookup at `update_overlay.rs:328` falls through to the locale fallback and renders English text on a Swedish UI. Add `BodyManualDownload=In-app-installation är inte tillgänglig på den här plattformen. Besök:` (or whatever wording the rest of the Swedish updater strings prefer). |
...
### M27. Apply success + relaunch failure looks like apply failure ✅ Done
